...
class MyWaitSaving(QtWidgets.QDialog, Ui_waitBatchWindow):
    def __init__(self, file_name, file_ext, open_file_ext, glob_attr, dim_list, var_dict):
        logging.debug('gui - saving_functions.py - MyWaitSaving - __init__')
    ...
    def launch_saving_thread(self):
        logging.debug('gui - saving_functions.py - MyWaitSaving - launch_saving_thread')
        self.save_thread = SaveFileThread(self.file_name, self.file_ext, self.open_file_ext,
                                          self.glob_attr, self.dim_list, self.var_dict)
    ...
    def saving_finished(self):
        logging.debug('gui - saving_functions.py - MyWaitSaving - saving_finished')
        self.close()

    def saving_failed(self, val):
        logging.debug('gui - saving_functions.py - MyWaitSaving - saving_failed')
    ...
    def setup_spinner(self):
        logging.debug('gui - saving_functions.py - MyWaitSaving - setup_spinner')
    ...
    def closeWindow(self):
        logging.debug('gui - saving_functions.py - MyWaitSaving - closeWindow')
        self.close()
