...
import pathlib
import re
from functions.plot_gd_option_gui_functions import add_figure_options, add_plot_options
from functions.plot_gd_option_secondary_functions import set_projection_function, wait_window, close_wait_window
from functions.plot_gd_option_secondary_functions import update_wait_window
from functions.thread_functions import DrawGriddedMap
        ...
        if str(self.pw_grid_combobox_14.currentText()) == 'HEX Color':
            if self.pw_grid_line_18.text():
                ...
                if '#' not in hex_code:
                    hex_code = '#' + hex_code
                if re.search(r'^#(?:[0-9a-fA-F]{3}){1,2}$', hex_code):
                    self.gd_plot_options['coast_fill_color'] = hex_code
                else:
                    self.gd_plot_options['coast_fill_color'] = self.gd_plot_options['default_coast_fill_color']
            else:
                self.gd_plot_options['coast_fill_color'] = self.gd_plot_options['default_coast_fill_color']
            ...
            try:
                ...
                self.gd_plot_options['coast_fill_color'] = (r, g, b)
            except ValueError:
                self.gd_plot_options['coast_fill_color'] = self.gd_plot_options['default_coast_fill_color']
        else:
            self.gd_plot_options['coast_fill_color'] = str(self.pw_grid_combobox_14.currentText()).lower()
